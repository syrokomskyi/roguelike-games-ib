...
/**
 * Create a test workspace with canonical records pre-populated.
 */
export function setupCanonicalWorkspace(options?: {
  kbId?: string;
  records?: TestRecord[];
  claims?: unknown[];
  relations?: unknown[];
  ...
  const workspace = createTestWorkspace({ kbId: options?.kbId ?? "mat-test" });
...
/**
 * Generate a UUIDv7-like ID for test records.
 */
