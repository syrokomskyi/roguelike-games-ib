...
import { buildSearchIndex } from "@roguelike-games-ib/search";
import type { SearchIndex } from "@roguelike-games-ib/search";
import { createMcpContext, type McpContext } from "@roguelike-games-ib/mcp";
...
export async function setupMcpWorkspace(options: {
  kbId?: string;
  records?: TestRecord[];
  claims?: unknown[];
  relations?: unknown[];
  ...
  const workspace = createTestWorkspace({ kbId: options.kbId ?? "mcp-test" });
  ...
  const ctx = await createMcpContext(matResult.distDir);
