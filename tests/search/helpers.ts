...
export async function setupSearchWorkspace(options: {
  kbId?: string;
  records?: TestRecord[];
  claims?: unknown[];
  relations?: unknown[];
  ...
  const workspace = createTestWorkspace({ kbId: options.kbId ?? "search-test" });
  ...
  const dbPath = join(matResult.distDir, "knowledge.sqlite");
  ...
  return {
    ...
    canonicalHash: matResult.canonicalHash,
    index,
    cleanup: () => cleanupTempWorkspace(workspace),
  };
}
